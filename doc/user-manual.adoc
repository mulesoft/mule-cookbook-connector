...
* Review the authentication options. In the *Choose Global Type* wizard, expand *Connector Configuration* and select one of the provided options:
...
Test out the operations inside a clean cookbook user interface, available to you after running one of the  links:https://github.com/mulesoft/cookbook-connector/tree/release/1.0.0/demo[demo Mule applications], for example `demo-crud-ingredient`, which uses the Cookbook connector.
